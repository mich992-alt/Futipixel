...
export const PixelCanvas: React.FC<PixelCanvasProps> = ({
  ...
  // Calibración temporal División 23: ~5.3-5.6s para cruzar el ancho del campo (692 px)
  const calibratedPlayerSpeed = 2.15 + (moveSpeedLevel - 1) * 0.15;
  ...
  const initDefenders = useCallback(() => {
    ...
    for (let i = 0; i < defCount; i++) {
      ...
      const individualSpeed = 1.68 + ((i * 7 + 0 * 13) % 10) * 0.032; // 1.68 a 1.97 px/f (~5.8s a 6.8s)
      defenders.push({
        id: idCounter++,
        row: 0,
        col: i,
    ...
    for (let i = 0; i < midCount; i++) {
      ...
      const individualSpeed = 1.70 + ((i * 7 + 1 * 13) % 10) * 0.031; // 1.70 a 1.98 px/f (~5.8s a 6.8s)
      defenders.push({
        id: idCounter++,
        row: 1,
        col: i,
    ...
    for (let i = 0; i < fwCount; i++) {
      ...
      const individualSpeed = 1.72 + ((i * 7 + 2 * 13) % 9) * 0.032; // 1.72 a 1.98 px/f
      defenders.push({
        id: idCounter++,
        row: 2,
        col: i,
  ...
  const shootBall = (isSuper: boolean = false, isCurved: boolean = false) => {
    ...
    const baseCooldown = isSuper ? 28 : isCurved ? 24 : 20;
    player.shotCooldown = Math.max(8, Math.round(baseCooldown - (fireRateLevel - 1) * 1.5));
    ...
    const vyBase = isSuper
      ? -13.2 - (shotVelocityLevel - 1) * 0.4
      : isCurved
      ? -10.5 - (shotVelocityLevel - 1) * 0.35
      : -9.8 - (shotVelocityLevel - 1) * 0.35;
    ...
    if (isSuper) {
      addFloatingText('⚡ ¡SUPER TIRO!', ballX - 30, ballY - 15, '#facc15', 16);
    } else if (isCurved) {
      addFloatingText('🌀 ¡TIRO CURVO!', ballX - 30, ballY - 15, '#38bdf8', 16);
    }
  };
  ...
  useEffect(() => {
    ...
    const gameLoop = (currentTime: number) => {
      ...
      if (!isPaused && snapshot.state !== 'PENALTY' && snapshot.state !== 'FREE_KICK') {
        const player = playerRef.current;
        ...
        if (player.superCharge < 100) {
          player.superCharge = Math.min(100, player.superCharge + 0.035);
        ...
        if (gk.diveState !== 'recovering') {
          if (incomingBall) {
            ...
            gk.x += Math.sign(gkDiff) * Math.min(gk.speed * 1.3, Math.abs(gkDiff));
          } else {
            ...
            if (gk.aiState !== 'PAUSED') {
              gk.x += gk.speed * 0.85 * gk.direction;
            }
          }
        ...
        for (let i = 0; i < defenders.length; i++) {
          ...
          const currentSpeed = def.baseSpeed || 1.8;
          def.x += currentSpeed * (def.direction || 1);
        ...
        for (let bIdx = balls.length - 1; bIdx >= 0; bIdx--) {
          ...
          if (ball.isCurvedShot) {
            ball.vx += ball.curve * 0.09;
          } else {
            ball.vx += ball.curve * 0.04;
          }
            ...
            if (
              ...
              if (ball.isSuperShot || ball.isCurvedShot) {
                if (ball.piercedCount < 2) {
                  ...
                  triggerSpeechBubble('¡¡KO!! ¡Qué zapatazo!', def.x, def.y);
                  addFloatingText(
                    `💥 ¡SUPERA DEFENSOR! (${ball.piercedCount}/2)`,
                    def.x - 25,
                    def.y - 12,
                  ...
                  triggerSpeechBubble('¡Frenado heroico!', def.x, def.y);
                  addFloatingText('⛔ ¡FRENADO AL 3ER JUGADOR!', def.x - 30, def.y - 10, '#ef4444', 14);
                ...
                if (ball.bounceCount < 2) {
                  ...
                  // Rebote físico hacia abajo/afuera del defensor (para no re-colisionar de inmediato)
                  ball.vy = Math.abs(ball.vy) * 0.65;
                  const hitSide = ball.x < def.x + def.width / 2 ? -1 : 1;
                  ball.vx = hitSide * (2.8 + Math.random() * 2.2);
                  ...
                  addFloatingText(`¡REBOTE! (${ball.bounceCount}/2)`, def.x - 10, def.y - 10, '#38bdf8', 13);
                  matchEngine.registerDefenderDeflection(String(def.id), false, false, false);
                } else {
                  ball.active = false;
                  ...
                  spawnTurfSparks(ball.x, ball.y, true);
                  retroAudio.playDefenderBlock();
                  triggerSpeechBubble('¡Mío el balón!', def.x, def.y);
                  addFloatingText('¡BALÓN PERDIDO!', def.x - 15, def.y - 10, '#ef4444', 15);
          ...
          if (
            ...
            if (gk.diveState === 'recovering') {
              // Arquero recuperándose: gol cantado
              ...
              retroAudio.playBounce();
              addFloatingText('¡QUEMÓ LOS GUANTES!', gk.x - 30, gk.y + 20, '#facc15', 14);
            } else {
              ball.active = false;
              ...
              retroAudio.playSave();
              spawnTurfSparks(ball.x, ball.y, false);
              addFloatingText('🧤 ¡ATAJADÓN!', gk.x - 20, gk.y + 20, '#38bdf8', 16);
          ...
          if (ball.y - ball.radius <= goal.y + goal.height - 10) {
            if (ball.x >= goal.x && ball.x <= goal.x + goal.width) {
              ...
              if (widthRatio > 0.75) {
                multiplier = 4;
                multiplierLabel = 'x4 GOLAZO AL ÁNGULO';
              ...
              addFloatingText(
                `¡¡GOOOOL!! +${totalPoints} PTS (${multiplierLabel})`,
              ...
              spawnTurfSparks(ball.x, ball.y, true);
              retroAudio.playBounce();
              addFloatingText('¡FUERA!', ball.x - 20, ball.y + 20, '#94a3b8', 14);
      ...
      for (const def of defenders) {
        if (def.isStunned) {
          ...
          ctx.fillText('💫', def.x + Math.cos(starAngle) * 12 + 6, def.y - 12 + Math.sin(starAngle) * 4);
        } else {
          renderProportionalCharacterSprite(ctx, {
            x: def.x,
            y: def.y,
    ...
    >
      ...
      {engineSnapshot.activeCounters.length > 0 && (
        <div className="absolute top-2 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 bg-gradient-to-r from-red-950 via-red-800 to-red-950 px-4 py-1.5 rounded-full border-2 border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.8)] animate-pulse">
          ...
          <span className="font-arcade text-xs text-white font-bold tracking-wider">
            {engineSnapshot.activeCounters[0].isGoal ? '⚡ ¡CONTRA RIVAL PELIGROSA!' : '⚡ ¡CONTRA RIVAL EN CURSO!'}
          </span>
        </div>
      )}

      {/* 2. RESUME COUNTDOWN OVERLAY ("2...", "1...", "¡YA!") */}
      {engineSnapshot.state === 'RESUME_COUNTDOWN' && (
        <div className="absolute inset-0 z-40 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center">
          ...
          <span className="text-sm font-arcade text-white mt-4 uppercase tracking-widest">¡REANUDANDO JUEGO!</span>
        </div>
      )}
        ...
        <div className="absolute inset-0 z-50 bg-black/85 backdrop-blur-md flex flex-col items-center justify-center p-4 text-white font-mono animate-fadeIn">
          <div className="w-full max-w-md bg-gradient-to-b from-[#2b1706] to-[#140b03] border-4 border-yellow-500 rounded-2xl p-5 shadow-2xl flex flex-col items-center text-center">
            ...
            <div className="grid grid-cols-3 gap-2 w-full mb-4">
              {(
                [
                  { id: 'TOP_LEFT', label: '↖ ARRIBA IZQ' },
                  { id: 'TOP_CENTER', label: '⬆ AL CENTRO' },
                  { id: 'TOP_RIGHT', label: '↗ ARRIBA DER' },
            ...
            {engineSnapshot.pendingPenalty.status === 'RESOLVED' && (
              <div className="font-arcade text-base font-bold text-emerald-400 animate-bounce">
                {engineSnapshot.pendingPenalty.isGoal ? '⚽ ¡¡GOOOOOOL DE PENALTI!!' : '🧤 ¡ATAJÓ EL ARQUERO!'}
              </div>
            )}
          </div>
        </div>
          ...
          <div className="w-full max-w-lg bg-gradient-to-b from-[#2b1706] to-[#140b03] border-4 border-cyan-500 rounded-2xl p-5 shadow-2xl flex flex-col items-center text-center">
            ...
            <div className="w-full mb-3 text-left">
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  { id: 'LEFT', label: '📐 POSTE IZQUIERDO' },
                  { id: 'CENTER', label: '🧱 POR ARRIBA DE BARRERA' },
                  { id: 'RIGHT', label: '📐 POSTE DERECHO' },
                ].map((z) => (
                  <button
                    key={z.id}
                    type="button"
            ...
            <div className="w-full mb-4 text-left">
              <span className="text-[10px] text-amber-300/80 uppercase font-bold block mb-1">2. TÉCNICA DE EJECUCIÓN:</span>
              <div className="grid grid-cols-3 gap-2">
                {[
                  { id: 'POWER', label: '⚡ POTENCIA PURA' },
                  { id: 'CURVE', label: '🌀 EFECTO CHANFLE' },
            ...
            {engineSnapshot.pendingFreeKick.status === 'RESOLVED' && (
              <div className="font-arcade text-base font-bold text-emerald-400 mt-2 animate-bounce">
                {engineSnapshot.pendingFreeKick.isGoal ? '⚽ ¡¡GOLAZO DE TIRO LIBRE!!' : '❌ EL TIRO SE FUE O FUE ATAJADO'}
              </div>
            )}
          </div>
        </div>
      ...
      <div className="w-full bg-[#120a03] p-1.5 px-3 border-t border-[#59300e] flex items-center justify-between text-[8px] font-mono text-amber-200/70">
        <div className="flex items-center gap-2">
          <span>MATCH ENGINE v1.0 • ESTADO: <strong className="text-cyan-300">{engineSnapshot.state}</strong></span>
          <span>•</span>
        ...
        >
          {showDebugOverlay ? 'OCULTAR TELEMETRÍA' : 'VER TELEMETRÍA QA'}
        </button>
      </div>
      ...
      {showDebugOverlay && (
        <div className="w-full bg-[#0a0502] p-2.5 text-[9px] font-mono text-cyan-300 border-t border-[#59300e] flex flex-col gap-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            <div>PRESIÓN: {engineSnapshot.rivalPressure.toFixed(1)}%</div>
          ...
          <div className="pt-1.5 border-t border-[#59300e]/60 grid grid-cols-1 sm:grid-cols-2 gap-1 text-[8px] text-yellow-200/90 bg-[#120803] p-2 rounded">
            <div><strong>PLAYER SPEED:</strong> {calibratedPlayerSpeed.toFixed(2)} px/frame (~{(692 / calibratedPlayerSpeed / 60).toFixed(1)}s cruce)</div>
            <div><strong>DIVISION SPEED MODIFIER:</strong> 1.00 (Div 23 Base)</div>
            <div><strong>FIRE RATE / COOLDOWN:</strong> {Math.max(8, Math.round(20 - (fireRateLevel - 1) * 1.5))} frames (Lvl {fireRateLevel})</div>
            <div><strong>SHOT VELOCITY:</strong> Normal {(-9.8 - (shotVelocityLevel - 1) * 0.35).toFixed(2)} | Super {(-13.2 - (shotVelocityLevel - 1) * 0.4).toFixed(2)} | Curvo {(-10.5 - (shotVelocityLevel - 1) * 0.35).toFixed(2)}</div>
            <div><strong>DEFENDER SPEED RANGE:</strong> 1.68 - 1.98 px/frame (~5.8s - 6.8s)</div>
